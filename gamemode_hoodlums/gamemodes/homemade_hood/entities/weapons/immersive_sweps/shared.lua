...
function SWEP:CanFire()
	...
	if requiresPump and timeSincePump > self.CycleTime then
		self:DoPump()
		return false
	end
...
function SWEP:DoPump()
	...
	if requiresPump and timeSincePump > 1 then
		self:SetRequiresPump(false)
		self:SetLastPump(CurTime())
...
function SWEP:Animate()
	...
	if bolt and CLIENT then
		if self.PumpAction then -- do pump n allat
			local lastPump = self:GetLastPump()
			local timeSincePump = CurTime() - lastPump
			local cycle = math.Clamp(timeSincePump / self.CycleTime, 0, 1)
	...
	-- shotgun pump
	if timeSinceLastPump < self.CycleTime then
		local pumpProgress = timeSinceLastPump / self.CycleTime
		self.angleupperL = LerpAngleFT(8 * FrameTime(), self.angleupperL, Angle(0, 15, 0))
		self.angleforeL = LerpAngleFT(8 * FrameTime(), self.angleforeL, Angle(0, 15, 0))
	end
